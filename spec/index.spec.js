...
describe('/', () => {
  it("Any request type will return an ERROR with a status code of 404 as this endpoint doesn't exist.", () => request
    .get('/')
    .expect(404)
    .then(({ body }) => {
...
describe('/api', () => {
  ...
  it('GET on /api endpoint RETURNS a JSON with all available endpoints.', () => request
    .get('/api')
    .expect(200)
    .then(({ body }) => {
  ...
  it('Returns an ERROR with a status code of 404 if an unrecognised endpoint is entered after /api.', () => request
  ...
  describe('/topics', () => {
    it('Returns an ERROR with a status code of 404 if an unrecognised endpoint is entered after /topics.', () => request
    ...
    it('Returns an ERROR with a status code of 422 if a user inserts a topic with a duplicate SLUG.', () => {
      const insertedTopic = {
        slug: 'mitch',
        description: 'test',
      };
      return request
        ...
        .expect(422)
        .then(({ body }) => {
          expect(body['Error 422']).to.equal('Unprocessable Entity');
        });
    });
    it('Returns an ERROR with a status code of 400 if a user inserts a topic without the right keys.', () => {
      ...
      return request
        ...
        .expect(400)
        .then(({ body }) => {
          expect(body['Error 400']).to.equal('Bad Request');
        });
    });
    it('Returns an ERROR with a status code of 405 if an invalid request is made by the user.', () => request
      .patch('/api/topics/')
      .expect(405)
      .then(({ body }) => {
  ...
  describe('/articles', () => {
    it('Returns an ERROR with a status code of 400 if an unrecognised endpoint is entered after /articles.', () => request
      .get('/api/articles/randomrubbish')
      .expect(400)
      .then(({ body }) => {
        expect(body['Error 400']).to.equal('Bad Request');
      }));
    it('Returns an ERROR with a status code of 405 if an unrecognised method is used on /articles.', () => request
      .patch('/api/articles')
      .send('something')
      .expect(405)
      .then(({ body }) => {
    ...
    it('GET articles can take a SORT BY query which ignores invalid columns.', () => request
    ...
    describe('/:article_id', () => {
      it('Returns an ERROR with a status code of 404 if an unrecognised article_id is entered after /articles.', () => request
        .get('/api/articles/900')
        .expect(404)
        .then(({ body }) => {
          expect(body['Error 404']).to.equal('Page Not Found');
        }));
      it('Returns an ERROR with a status code of 400 if the user enters an article_id that is not an integer.', () => {
        request
      ...
      it('GET using the article_id endpoint will return a 404 error if the ID passed in the request parameter does not match an article_id that exists.', () => request
        .get('/api/articles/900')
        .expect(404)
        .then(({ body }) => {
          expect(body['Error 404']).to.equal('Page Not Found');
        }));
      it('PATCH using the article_id endpoint will deal with a vote request to add votes. It will ignore the request if no value is passed into inc_votes and return a 400 error code.', () => request
        ...
        .then(() => request
          ...
          .expect(200)
          .then(({ body }) => {
            expect(body.article.votes).to.equal(11);
          }))
        .then(() => request
          .patch('/api/articles/6')
          .send({ inc_votes: null })
          .expect(400)
          .then(({ body }) => {
            expect(body['Error 400']).to.equal(
              'Your Vote Must Be A Number',
            );
          }))
        .then(() => request
          .patch('/api/articles/6')
      ...
      it('DELETE using the article_id endpoint will remove the article that matches the ID passed in the request parameter.', () => request
      ...
      describe('/comments', () => {
        ...
        it('GET on article_id/comments can take a SORT BY query which sorts by any valid column, and defaults to date if no column is queried.', () => request
        ...
        it('GET on article_id/comments will SORT BY author if the query is passed defaulting to descending.', () => request
  ...
  describe('/comments/:comment_id', () => {
    it('PATCH using the comment_id endpoint will deal with a vote request to add a vote.', () => request
    ...
    it('PATCH using the comment_id endpoint will deal with a vote request to subtract a vote.', () => request
      .patch('/api/comments/1')
      .send({ inc_votes: -1 })
      .expect(200)
      .then(({ body }) => {
    ...
    it('DELETE using the comment_id endpoint will remove the comment that matches the ID passed in the request parameter.', () => request
  ...
  describe('/users', () => {
    it('GET request on users endpoint will return all users with the keys of username, avatar_url and name.', () => request
      .get('/api/users')
      .expect(200)
      .then(({ body }) => {
